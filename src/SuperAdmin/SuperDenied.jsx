...
const SuperDenied = () => {
  ...
  useEffect(() => {
    const fetchDeniedData = async () => {
      try {
        ...
        const snapshot = await get(deniedRef);
        ...
        console.error("Error fetching denied data:", error);
      }
    };
  ...
  const handleArchive = async (id, name, contact, category, email) => {
    if (window.confirm("Are you sure you want to archive this item?")) {
      try {
        ...
        // Get the snapshot of denied data
        const snapshot = await get(deniedRef);
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          if (childData.id === id) {
            const itemRef = ref(db, `Denied/${childSnapshot.key}`);

            // Remove the item from the Denied node
            remove(itemRef);
        ...
        setDeniedData((prevData) => prevData.filter((item) => item.id !== id));
  ...
  // Filter deniedData based on searchQuery
  const filteredDeniedData = deniedData.filter((item) =>
  ...
  return (
    <div className="flex">
      ...
      <div className="ml-64 p-8 w-full">
        <h1 className="text-3xl mb-6 font-semibold">Denied Requests</h1>
